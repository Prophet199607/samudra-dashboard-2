          ...
          <div className='border-t border-gray-200 p-2'>
            ...
            <div className='flex items-center justify-start sm:justify-center overflow-x-auto scrollbar-hide mt-2'>
              {TAB_CONFIG.map((tab, index) => {
                const isCompleted = savedSteps.has(tab.id);
                const isDisabled = disabledSteps.has(tab.id);
                const isBusinessDisabled = businessDisabledSteps.has(tab.id);
                const isActive = activeTab === tab.id;
                return (
                  <div key={tab.id} className='flex items-center'>
                    ...
                    >
                      ...
                      <div className='flex flex-col items-center gap-1 mt-1.5'>
                        {isCompleted && !isBusinessDisabled && (
              ...
              <div className='flex space-x-2 w-full sm:w-auto justify-center'>
                {(() => {
                  ...
                  const isPreviousStepsCompleted = () => {
                    ...
                    // Find all previous steps that are not business-disabled
                    const requiredPreviousSteps = TAB_CONFIG.filter(
                      (tab) =>
                        tab.id < activeTab && !businessDisabledSteps.has(tab.id)
                    );

                    // Check if all required previous steps are saved
                    return requiredPreviousSteps.every((tab) =>
                      savedSteps.has(tab.id)
                    );
                  };
